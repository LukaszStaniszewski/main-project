...
export const createCollectionWithItems = async (
   body: ICollectionWithItems,
   image?: Buffer
) => {
   try {
...
export const appendItemsToCollections = (
   collections: IItemCollectionDocument[] extends IAppendItemsToCollections[]
      ? IItemCollectionDocument[]
      : IAppendItemsToCollections[],
   items: IItemDocument[]
): IAppendItemsToCollections[] => {
   for (let collection = 0; collection < collections.length; collection++) {
      for (let item = 0; item < items.length; item++) {
         if (
            collections[collection]["_id"].toString() ===
            items[item]["collectionId"].toString()
         ) {
